...
class RestRouterExtension extends CompilerExtension
{
	...
	public function beforeCompile(): void
	{
		...
		$builder->addDefinition($this->prefix('resolver'))
			->setFactory(ApiRoutesResolver::class)
			->addSetup('prepandRoutes', [$builder->getDefinition('router'), $routes])
			->addTag('run');

		$builder->addDefinition($this->prefix('annotationsResolver'))
			->setFactory(AnnotationsResolver::class)
	...
	private function setupReaderAnnotations(array $config): void
	{
		 ...
		 */
		AnnotationRegistry::loadAnnotationClass(ApiRoute::class);
		AnnotationRegistry::loadAnnotationClass(ApiRouteSpec::class);
	...
	{
		$cache_path = $compiler_config['parameters']['tempDir'] . '/cache/Packeto.RestRouter.Annotations';
	...
	{
		$presenter = ClassType::from($presenter);
		$route = $this->reader->getClassAnnotation($presenter, ApiRoute::class);
		...
		$route->setDescription($presenter->getAnnotation('description'));

		if (!$route->getPresenter()) {
			$route->setPresenter(preg_replace('/Presenter$/', '', $presenter->getShortName()));
		}

		/**
		 * Find apropriate methods
		 */
		foreach ($presenter->getMethods() as $method_r) {
